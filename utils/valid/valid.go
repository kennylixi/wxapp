...
// 初始化错误消息管理对象
func init() {
	batchSet(defaultMessages)
}

func batchSet(m map[string]string) {
	for k, v := range m {
		errorMsgMap[k] = v
	}
}
...
// 判断必须字段
func checkRequired(value, rulekey, ruleval string, params map[string]interface{}) bool {
	required := false
	switch rulekey {
	// 必须字段
	case "required":
		...
		if len(array)%2 == 0 {
			for i := 0; i < len(array); {
				...
				if v, ok := params[tk]; ok {
					if strings.Compare(tv, conv.String(v)) == 0 {
						required = true
						break
					}
				}
...
func checkIpv4(value string) bool {
	return regx.IsMatchString(`^((25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(25[0-5]|2[0-4]\d|[01]?\d\d?)$`, value)
}

func checkIpv6(value string) bool {
	return regx.IsMatchString(`^([\da-fA-F]{1,4}:){7}[\da-fA-F]{1,4}$|^:((:[\da-fA-F]{1,4}){1,6}|:)$|^[\da-fA-F]{1,4}:((:[\da-fA-F]{1,4}){1,5}|:)$|^([\da-fA-F]{1,4}:){2}((:[\da-fA-F]{1,4}){1,4}|:)$|^([\da-fA-F]{1,4}:){3}((:[\da-fA-F]{1,4}){1,3}|:)$|^([\da-fA-F]{1,4}:){4}((:[\da-fA-F]{1,4}){1,2}|:)$|^([\da-fA-F]{1,4}:){5}:([\da-fA-F]{1,4})?$|^([\da-fA-F]{1,4}:){6}:$`, value)
}

// 检测键值对参数Map，注意返回参数是一个2维的关联数组，第一维键名为参数键名，第二维为带有错误的校验规则名称，值为错误信息
func CheckMap(params map[string]interface{}, rules map[string]string, msgs ...map[string]interface{}) map[string]string {
	// 自定义消息，非必须参数，因此这里需要做判断
	cmsgs := make(map[string]interface{})
	if len(msgs) > 0 {
		cmsgs = msgs[0]
	}
	emsgs := make(map[string]string)
	for key, value := range params {
		if rule, ok := rules[key]; ok {
			required := strings.Contains(rule, "required")
			valueStr := conv.String(value)
			//如果没有required的验证，那么没有数据时就不执行验证
			if required == false && len(valueStr) <= 0 {
				continue
			}
			msg, _ := cmsgs[key]
			if m := Check(valueStr, rule, msg, params); len(m) > 0 {
				emsgs[key] = m
			}
		}
	}
...
func Check(value, rules string, msgs interface{}, params ...map[string]interface{}) string {
	...
	for index := 0; index < len(items); {
		...
		case "regex":
			...
			for i := index + 1; i < len(items); i++ {
				...
				if !regx.IsMatchString(gSINGLE_RULE_PATTERN, items[i]) {
					ruleval += "|" + items[i]
					index++
				}
			}
			match = regx.IsMatchString(ruleval, value)

		// 日期格式，
		case "date":
			for _, v := range []string{"2006-01-02", "20060102", "2006.01.02"} {
				if _, err := time.StrToTime(value, v); err == nil {
					match = true
					break
				}
			}
		...
		// 日期格式，需要给定日期格式
		case "date-format":
			if _, err := time.StrToTime(value, ruleval); err == nil {
				match = true
			}
		...
		case "same":
			if v, ok := kvmap[ruleval]; ok {
				if strings.Compare(value, conv.String(v)) == 0 {
					match = true
				}
			}
		 ...
		 */
		case "phone":
			match = regx.IsMatchString(`^13[\d]{9}$|^14[5,7]{1}\d{8}$|^15[^4]{1}\d{8}$|^17[0,3,5,6,7,8]{1}\d{8}$|^18[\d]{9}$`, value)

		// 国内座机电话号码："XXXX-XXXXXXX"、"XXXX-XXXXXXXX"、"XXX-XXXXXXX"、"XXX-XXXXXXXX"、"XXXXXXX"、"XXXXXXXX"
		case "telephone":
			match = regx.IsMatchString(`^((\d{3,4})|\d{3,4}-)?\d{7,8}$`, value)

		// 腾讯QQ号，从10000开始
		case "qq":
			match = regx.IsMatchString(`^[1-9][0-9]{4,}$`, value)

			// 中国邮政编码
		case "postcode":
			match = regx.IsMatchString(`^[1-9]\d{5}$`, value)
		...
		*/
		case "id-number":
			match = regx.IsMatchString(`(^[1-9]\d{5}(18|19|([23]\d))\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{3}[0-9Xx]$)|(^[1-9]\d{5}\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{2}$)`, value)

		// 通用帐号规则(字母开头，只能包含字母、数字和下划线，长度在6~18之间)
		case "passport":
			match = regx.IsMatchString(`^[a-zA-Z]{1}\w{5,17}$`, value)

		// 通用密码(任意可见字符，长度在6~18之间)
		case "password":
			match = regx.IsMatchString(`^[\w\S]{6,18}$`, value)

		// 中等强度密码(在弱密码的基础上，必须包含大小写字母和数字)
		case "password2":
			if regx.IsMatchString(`^[\w\S]{6,18}$`, value) && regx.IsMatchString(`[a-z]+`, value) && regx.IsMatchString(`[A-Z]+`, value) && regx.IsMatchString(`\d+`, value) {
				match = true
			}

		// 强等强度密码(在弱密码的基础上，必须包含大小写字母、数字和特殊字符)
		case "password3":
			if regx.IsMatchString(`^[\w\S]{6,18}$`, value) && regx.IsMatchString(`[a-z]+`, value) && regx.IsMatchString(`[A-Z]+`, value) && regx.IsMatchString(`\d+`, value) && regx.IsMatchString(`\S+`, value) {
				match = true
			}

		// json
		case "json":
			if _, err := json.Decode([]byte(value)); err == nil {
				match = true
			}
		...
		// 邮件
		case "email":
			match = regx.IsMatchString(`^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$`, value)

		// URL
		case "url":
			match = regx.IsMatchString(`^(?:([A-Za-z]+):)?(\/{0,3})([0-9.\-A-Za-z]+)(?::(\d+))?(?:\/([^?#]*))?(?:\?([^#]*))?(?:#(.*))?$`, value)

		// domain
		case "domain":
			match = regx.IsMatchString(`^([0-9a-zA-Z][0-9a-zA-Z-]{0,62}\.)+([0-9a-zA-Z][0-9a-zA-Z-]{0,62})\.?$`, value)

		// IP(IPv4/IPv6)
		case "ip":
			match = checkIpv4(value) || checkIpv6(value)

		// IPv4
		case "ipv4":
			match = checkIpv4(value)

		// IPv6
		case "ipv6":
			match = checkIpv6(value)

		// MAC地址
		case "mac":
			match = regx.IsMatchString(`^([0-9A-Fa-f]{2}-){5}[0-9A-Fa-f]{2}$`, value)

		default:
			errmsg = "Invalid rule name:" + rulekey
		}
